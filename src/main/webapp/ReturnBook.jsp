    ...
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item active dropdown mr-3">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">       
          ...
          <a class="dropdown-item" href="#"><i class="fa fa-backward" aria-hidden="true"></i> Return Book</a>
...
<div class="jumbotron text-center mb-0 mt-2">
  <h1 class="display-4" >Return Book</h1>
...
<table class="table table-striped container text-center mt-4">
  ...
  <a href="" id="returnBookConfirmLink" style="visibility:hidden" data-toggle="modal" data-target="#returnBookConfirmModal">Return Book Confirm Modal</a>
  <a href="" id="bookReturnedLink" style="visibility:hidden" data-toggle="modal" data-target="#bookReturnedModal">Book returned</a>
  <tbody>
  <c:forEach items="${issueBookHistory}" var="issueBookHistory">
	   	<c:url var="returnBookModal" value="/returnBookModal">
			<c:param name="issueId" value="${issueBookHistory.getIssueid()}" />
		</c:url>
		    <tr>
		      <td>${issueBookHistory.getIssueid()}</td>
		      <td>${issueBookHistory.getBookDetails().getBookid()}</td>
		      <td>${issueBookHistory.getBookList().getBarcode()}</td>
		      <td>${issueBookHistory.getStudentDetails().getUsername()}</td>
		      <td>${issueBookHistory.getIssuedate()}</td>
		      <c:if test="${issueBookHistory.getTotaldays()<=6}">
		      	<td>${issueBookHistory.getTotaldays()}</td>
		      </c:if>
		      <c:if test="${issueBookHistory.getTotaldays()==7}">
		      	<td class="badge badge-warning badge-pill mt-2">${issueBookHistory.getTotaldays()}</td>
		      </c:if>
		      <c:if test="${issueBookHistory.getTotaldays()>7}">
		      	<td class="badge badge-danger badge-pill mt-2">${issueBookHistory.getTotaldays()}</td>
		      </c:if>
		      <td><a href="${returnBookModal}"><button class="btn btn-danger btn-sm">Return</button></a></td>
		    </tr>
    </c:forEach>
  </tbody>  
</table>
...
<c:if test="${status == null }">
	<div class="mt-4 text-dark text-center" style="">
		<h2>All books have been returned!</h2>
	</div>
</c:if>
...
<div class="modal fade" id="returnBookConfirmModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body text-center" style="font-weight: bold">
		...
		<form:form action="returnBookConfirm" method="post">
			<input name="issueId" type="number" style="visibility:hidden" value="${issueid}">
			<button type="submit" class="btn btn-success" style="width:70px; position:absolute; left:170px">Yes</button>
			<button type="button" onclick="Redirect()" class="btn btn-danger" data-dismiss="modal" style="width:70px; margin-right:100px">No</button>
		</form:form>
      </div>
    </div>
  </div>
				...
				<!-- Book Returned Modal -->
<div class="modal fade" id="bookReturnedModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
...
<c:if test="${openModal != null}">
	<script>
		document.getElementById("returnBookConfirmLink").click();
	</script>
</c:if>	

<c:if test="${returnDone != null}">
	<script>
		document.getElementById("bookReturnedLink").click();
	</script>
</c:if>	
...
<script type="text/javascript">
	function Redirect()
	{
		window.location="https://library-portal.herokuapp.com/returnBook";
	}	
</script>
